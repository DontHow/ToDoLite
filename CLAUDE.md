...
**办他**（TodoLite）是一个 Local First 的原生 SwiftUI 待办应用，支持 iOS 和 macOS。数据以独立 JSON 文件形式存储在 iCloud Documents 中（不使用 SwiftData / Core Data）。每个待办、项目和标签都是单独的 JSON 文件。SQLite 仅用于全文搜索索引，不作为事实源。
...
| 层级 | 文件 | 职责 |
|------|------|------|
| 状态 | `Shared/Store/TodoStore.swift` | `@Observable` 单例。持有 `todos`、`projects`、`tags`、`focusSet`、`llmConfig`、`fontSizeLevel`。所有 CRUD 为异步操作，Repository 确认成功后更新 store。 |
| 仓库 | `Shared/Repository/TodoRepository.swift`、`ProjectRepository.swift`、`TagRepository.swift`、`FocusRepository.swift`、`LLMConfigRepository.swift` | `actor` 单例。写入前进行版本号冲突检测。 |
...
| 解析器 | `Shared/Parser/TodoParser.swift`、`DateResolver.swift` | 基于规则的快速录入解析器。语法：`@项目`、`#标签`、`!优先级`、`^日期`。 |
| LLM | `Shared/LLM/LLMService.swift`、`LLMParser.swift` | 可选 AI 解析。`LLMService` 调用 OpenAI 兼容 API 将自然语言转为结构化数据。`LLMParser` 负责 prompt 构造与 JSON 提取。 |
...
| **Suggested** | `dueAt` 为今天 | 系统建议，不自动进入 Focus |
| **Overdue** | `dueAt` < 今天 | 逾期任务，不污染 Focus |
| **Upcoming** | `dueAt` > 今天 | 未来截止任务，按日期排序 |

所有区域默认按 **项目分组**（`TaskListView` 的 `defaultGrouping: .byProject`）。
...
- `TodoStore` 派生集合（`focusTodos`、`suggestedTodos`、`overdueTodos`、`upcomingTodos`、`activeTodos`、`inboxTodos`）及排序
- 旧版 JSON 字符串的状态 Codable 迁移

## UI 结构

- **iOS**：`TabView`，标签页：今日、收件箱、看板、搜索、设置
- **macOS**：`NavigationSplitView`，Sidebar → 详情。包含 `MenuBarExtra`。快捷键：⌘1 今日、⌘2 收件箱、⌘3 看板、⌘K 搜索

Sidebar 视图列表：今日、看板、项目、标签、搜索、已完成、已归档、设置
...
4. **AI 仅用于输入解析辅助。** `LLMParser` 将自然语言转为结构化草稿，最终仍走 `TodoStore` → Repository 的持久化路径。AI 不直接写入文件系统。默认解析器为基于规则的 `TodoParser`。
...
5. **Parser + Search** — Quick Entry / SQLite FTS / LLM 解析
6. **体验层** — 快捷键 / MenuBar / Widget / 动画 / 字体大小
...
- **禁止第三方依赖。** 使用纯 SwiftUI。编写模块化、高度可复用的子视图（如 `TaskRow`、`CategoryCard`）。确保代码在 Swift 5.9 下编译。
